...
try:
    df = pd.read_csv('z_task4_results_2000.txt', names=headers)
...
# --- Gráfica 2: Mean Error ATE per Run (NUEVA) ---
ax2.bar(df['Run'], df['Mean'], color='mediumseagreen', edgecolor='darkgreen')
ax2.axhline(df['Mean'].mean(), color='darkorange', linestyle='--', 
            label=f'Avg Mean: {df["Mean"].mean():.4f}m')
ax2.set_title('Mean Error ATE per Run', fontsize=14, fontweight='bold')
ax2.set_xlabel('Execution Run #', fontsize=12)
ax2.set_ylabel('Mean Error (meters)', fontsize=12)
ax2.set_xticks(df['Run'])
ax2.legend()
ax2.grid(axis='y', linestyle=':', alpha=0.7)

# --- Gráfica 3: Trajectory Length per Run ---
ax3.bar(df['Run'], df['Pairs'], color='salmon', edgecolor='darkred')
ax3.axhline(df['Pairs'].mean(), color='blue', linestyle='--', 
            label=f'Mean Length: {df["Pairs"].mean():.2f}')
ax3.set_title('Trajectory Length per Run', fontsize=14, fontweight='bold')
ax3.set_xlabel('Execution Run #', fontsize=12)
ax3.set_ylabel('Number of Pairs (Frames)', fontsize=12)
ax3.set_xticks(df['Run'])
ax3.legend()
ax3.grid(axis='y', linestyle=':', alpha=0.7)

# Ajuste y guardado
plt.tight_layout()
plt.savefig('Task4_stats_2000_ATE.png', dpi=300)
